...
export const getJobsByCompanyAddress = async () => {
  let conn

  try {
    conn = await db.getConnection()

    const rows = await conn.query("SELECT DISTINCT company_wallet_address FROM jobs")

    const distinctCompanyAddressByJobs = rows?.map(row => row.company_wallet_address)

    let jobsByCompanyAddress = []

    if(distinctCompanyAddressByJobs.length === 0) return []

    let jobsPromises = []

    for (const companyAddress of distinctCompanyAddressByJobs) {
      jobsPromises[companyAddress] = await conn.query(
        {
          namedPlaceholders: true,
          sql: "SELECT * from jobs WHERE company_wallet_address = :companyAddress",
        }, {
          companyAddress
        }
      )
    ...
    return await Promise.all(
      distinctCompanyAddressByJobs.map(companyAddress => jobsPromises[companyAddress])
    )
  } catch (error) {
    return { error }
  }
...
export const insertJob = async ({ title, description, rate, transactionId, walletAddress }) => {
  let conn
  ...
  try {
    ...
    await conn.query({
        namedPlaceholders: true,
        sql:`
          INSERT INTO jobs (company_wallet_address, title, description, rate, transaction_id)
          VALUES (:walletAddress, :title, :description, :rate, :transactionId)
        `
      }, {
        title,
        description,
